...
namespace CI_Platform_Web.Controllers
{
	...
	public class VolunteeringMissionController : Controller
	{
		...
		public IActionResult Index(long? id)
		{
			...
			vmModel.MissionModel = HomeController.convertDataModelToMissionModel(_volunteeringMissionRepository.getCurrentMissionDetails(id), vmModel.user.UserId);
			return View(vmModel);
		}
		...
		public IActionResult getAllRelatedMissions(long missionId, long userId)
		{
			...
			if (missions.Count() > 0)
			{
				...
				{
					topThreeMissions.Add(HomeController.convertDataModelToMissionModel(currMission, userId));
				}
			}
			return PartialView("_RelatedMissions", topThreeMissions);
		}
